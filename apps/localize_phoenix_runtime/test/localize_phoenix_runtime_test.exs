defmodule Localize.PhoenixRuntime.PhoenixRuntimeTest do
  use ExUnit.Case
  ...
  describe "plug/3" do
    test "expands the runtime attributes and returns a conn" do
      ...
      %Plug.Conn{} = returned_conn = PhoenixRuntime.plug(conn, [], attrs)
      assert returned_conn.private.loc == expected
    end
  end
  ...
  describe "parse accept language" do
    test "with language only fallbacks" do
      ...
      assert expected == PhoenixRuntime.Parser.parse_accept_language(value)

      value1 = "*"
      expected1 = []
      assert expected1 == PhoenixRuntime.Parser.parse_accept_language(value1)

      value2 = "en"
      expected2 = [%{language: "en", locale: "en", quality: 1.0, territory: nil, region: nil}]
      assert expected2 == PhoenixRuntime.Parser.parse_accept_language(value2)
      ...
      assert expected3 == PhoenixRuntime.Parser.parse_accept_language(value3)
    end
  end
end
