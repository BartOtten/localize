defmodule Localize.PhoenixRuntime.ExtractorTest do
  use ExUnit.Case, async: true
  alias Localize.PhoenixRuntime.Extractor
  alias Plug.Conn
  ...
  defmodule Localize.PhoenixRuntime.Registry do
  ...
  setup do
    conn =
      ...
      |> Phoenix.ConnTest.init_test_session(%{loc: %{"locale" => "nl-BE"}})
  ...
  describe "extract_from_source/4 with Plug.Conn - session" do
    test "extracts from session", %{conn: conn} do
      conn = conn |> Phoenix.ConnTest.init_test_session(%{loc: %{"locale" => "en-US"}})
