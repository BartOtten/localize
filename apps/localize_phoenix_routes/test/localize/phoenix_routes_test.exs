defmodule Localize.PhoenixRoutesTest do
  use ExUnit.Case
  ...
  alias Localize.PhoenixRoutes

  describe "configure/2" do
    test "adds alternative generating extension" do
      result = PhoenixRoutes.configure(DummyOpts.opts(), DummyBackend)
    ...
    test "locale_route_prefix with single tag" do
      ...
      for {primary_prefix, expected_prefixes} <- kv do
        ...
        result = PhoenixRoutes.configure(opts, DummyBackend)
    ...
    test "locale_route_prefix with dual tags (fallback mechanism)" do
      ...
      result2 = PhoenixRoutes.configure(opts2, DummyBackend)
      expected2 = ["/fr", "/nl"]
